            ...
            {state.form === 'chat' && (
              <>
                ...
                <div className="bg-[var(--bg-card)] px-3 py-2.5 flex items-center gap-2 shrink-0 border-t border-[var(--border-light)]">
                  ...
                  {!state.isEscalated && !state.isClosed && state.conversationId && (
                    <a
                      href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '573001234567'}?text=${encodeURIComponent('Hola, necesito ayuda con mi reserva de LocalPlug')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      title={t.chatWidget.whatsappTooltip || 'Chat on WhatsApp'}
